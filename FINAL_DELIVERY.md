...
- `Q02`：通过 mock `ssh` 在沙盒中验证登录参数，并要求执行 `exit` 完成退出
- `Q03-Q09`：在受控工作目录中真实执行命令，并比对输出/文件结果
- `Q10`：真实执行 shell 脚本并校验输出，同时保留硬编码答案拦截
- `API_DEMO`：在沙盒中执行 Python 代码并调用指定函数
- 文件型题已支持按题目配置输入文件只读 / 可写权限，并对只读文件做执行前后快照比对，防止通过临时文件替换等方式绕过限制
...
> 我们组负责的 B3 模块已经完成题库管理、动态判题和评测结果落库的完整闭环。原先只能通过静态规则判断答案，现在已经支持在 Ubuntu 环境下通过沙盒真实运行代码后再判题，并且已经接入 MySQL 数据库和 Docker 容器沙盒。对于文件题，系统还能根据题目要求控制输入文件只读或可写，并通过执行前后快照比对防止违规修改输入文件。
